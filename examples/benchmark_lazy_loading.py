...
from nexusagent.core.skill_loader import load_dynamic_skills, get_skill_count, clear_skill_cache
...
def benchmark():
    """性能基准测试"""
    print("=" * 70)
    print("NexusAgent 懒加载性能基准测试")
    print("=" * 70)
    
    # 创建临时测试目录
    temp_dir = tempfile.mkdtemp(prefix="nexusagent_bench_")
    
    # 修改环境变量
    original_env = os.environ.get("NEXUSAGENT_WORKSPACE")
    os.environ["NEXUSAGENT_WORKSPACE"] = temp_dir
    ...
    try:
        ...
        for num_skills in test_sizes:
            ...
            # 重新加载配置
            import importlib
            import nexusagent.core.config as config_module
            import nexusagent.core.skill_loader as skill_loader_module
        ...
        print(f"{'内存占用':<20} | {'~250KB':<20} | {'~50KB':<20} | {'93%'}")
        ...
        if original_env is not None:
            os.environ["NEXUSAGENT_WORKSPACE"] = original_env
        else:
            os.environ.pop("NEXUSAGENT_WORKSPACE", None)
