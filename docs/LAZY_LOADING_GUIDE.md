...
NexusAgent 实现了**渐进式加载 + 缓存**的技能加载机制，显著提升了系统的启动速度和内存使用效率。
...
|------|------------------|------------------|------|
| **启动时间** | ~200ms | ~2.55ms | ⬇️ 98.7% |
| **内存占用** | ~250KB | ~50KB | ⬇️ 93% |
...
```python
from nexusagent.core.skill_loader import load_dynamic_skills, get_skill_count
...
```python
from nexusagent.core.skill_loader import reload_skills
...
```python
from nexusagent.core.skill_loader import clear_skill_cache
...
```python
from nexusagent.core.skill_loader import clear_skill_cache, reload_skills
...
```python
from nexusagent.core.skill_loader import _lazy_loader
